...
class SliceSequence:
    ...
    def add_slice(self, slice_index: SliceIndexType) -> None:
        ...
        if slice_index not in self.slices.index:
            ...
            self.slices = pd.concat([self.slices, new_row]).sort_index()
            self.slices['NextSlice'] = self.slices['ThisSlice'].shift(-1)
            self.slices['PreviousSlice'] = self.slices['ThisSlice'].shift(1)

    def remove_slice(self, slice_index: SliceIndexType) -> None:
        '''Remove a slice index from the sequence.'''
        if slice_index in self.slices.index:
            self.slices = self.slices.drop(slice_index)
            self.slices['NextSlice'] = self.slices['ThisSlice'].shift(-1)
            self.slices['PreviousSlice'] = self.slices['ThisSlice'].shift(1)

    def get_nearest_slice(self, value: float) -> SliceIndexType:
        '''Find the nearest slice index to a given value.'''
        return self.slices.index[(self.slices.index - value).abs().argmin()]
    ...
    def get_neighbors(self, slice_index: SliceIndexType) -> Union[SliceNeighbours, None]:
        ...
        if slice_index not in self.slices.index:
            return None
        row = self.slices.loc[slice_index]
    ...
    def __getitem__(self, index: int) -> SliceIndexType:
        return self.slices.iloc[index]['ThisSlice']

    def __iter__(self):
        return iter(self.slices['ThisSlice'])

    def __contains__(self, slice_index: SliceIndexType) -> bool:
        return slice_index in self.slices.index
