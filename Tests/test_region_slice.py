...
class TestRegionSliceMultiContour:
    '''Test cases for the RegionSlice class whether multiple regions are present
    on the same slice, and how boundary slices are created in that scenario.'''
    ...
    def test_region_slice_three_different_regions_on_same_slice(self):
        '''Test a complex scenario where three different regions overlap on the
        same slice and a boundary slice is created for one of them, the other
        regions are also present on the slice.'''
        # Box1, box2, box3 are offset horizontally (x = -5, x = +2.5, x = 5),
        # but overlap vertically (slices 0 to 4) vs (slices -3 to 1)
        # The contained_box is inside box2.
        # A boundary slice should be created at -0.5 for box1.,
        # box2 and box3 should also be present on that slice.
        box1 = box_points(width=4, offset_x=-5)
        ...
        assert structure_set.slice_sequence.sequence.at[-0.5,'Original'] == False
        # The region should have one non-empty boundary contour and
        # two non-empty regular contours.
        region_slice = structure_set.structures[1].get_slice(-0.5)
        boundaries = [contour for contour in region_slice.boundaries.values()
                    if not contour.is_empty]
        regions = [region for region in region_slice.regions.values()
        ...
        # Contour indexes should contain two ContourIndexes
        assert len(region_slice.contour_indexes) == 3
